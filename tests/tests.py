...
def test_word_no_neighbours() -> None:
...
def test_mismatch_word_chain() -> None:
    """WordChain should take list of words and build the graph. Get empty list if no chain exists."""
    chainer = WordChainer(word_list=WORD_LIST)
...
def test_word_chain_fails_when_words_have_different_lengths() -> None:
...
def test_word_chain_from_file() -> None:
...
def test_rubbish_chain_from_file() -> None:
...
def test_word_chain_collection() -> None:
    """WordChainCollection should take list of words and build the graph and chains for the different lengths."""
